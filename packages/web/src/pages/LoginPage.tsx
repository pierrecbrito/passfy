...
export const LoginPage: React.FC = () => {
  ...
  const handleSocialLogin = (provider: string) => {
    setToastMessage(`A autenticação com ${provider} ainda não foi implementada.`);
    setTimeout(() => {
    ...
    <div className="min-h-screen w-full bg-white flex flex-col lg:grid lg:grid-cols-12 p-3 sm:p-4 gap-4 sm:gap-6 shadow-none border-none relative overflow-y-auto lg:overflow-hidden select-none selection:bg-[#2b55f5] selection:text-white">
          ...
          <div className="flex flex-wrap items-center gap-4 sm:gap-6 text-white/90 text-xs font-bold">
            ...
            <div className="flex items-center gap-1.5 opacity-85 hover:opacity-100 transition">
              <svg className="w-4 h-4 fill-current text-[#1DB954]" viewBox="0 0 24 24">
                <path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.494 17.306c-.215.352-.674.464-1.026.249-2.812-1.718-6.353-2.107-10.523-1.155-.403.092-.803-.16-.895-.563-.092-.403.16-.803.563-.895 4.542-1.037 8.448-.598 11.632 1.348.352.215.464.674.249 1.016zm1.464-3.256c-.27.441-.849.58-1.29.31-3.22-1.979-8.13-2.553-11.939-1.396-.497.151-1.025-.133-1.176-.63-.151-.497.133-1.025.63-1.176 4.354-1.321 9.771-.676 13.465 1.593.441.27.58.849.31 1.299zm.126-3.39c-3.86-2.292-10.231-2.504-13.916-1.385-.592.18-1.22-.158-1.4-.75-.18-.592.158-1.22.75-1.4 4.237-1.286 11.278-1.037 15.719 1.597.532.316.707 1.008.391 1.54-.316.533-1.008.708-1.544.398z" />
              </svg>
      ...
      <div className="lg:col-span-6 h-auto lg:h-full flex flex-col justify-center items-center px-4 sm:px-8 lg:px-12 py-6 sm:py-8 bg-white overflow-y-auto">
        <div className="w-full max-w-[420px] mx-auto">
          ...
          {/* Botões Sociais Google & Apple Lado a Lado */}
          <div className="grid grid-cols-2 gap-3 mt-3">
            <button
              type="button"
              onClick={() => handleSocialLogin('Google')}
              className="flex items-center justify-center gap-2 py-2.5 px-3 rounded-lg border border-slate-300 bg-white hover:bg-slate-50 text-sm font-semibold text-slate-700 transition shadow-xs active:scale-[0.99]"
            >
